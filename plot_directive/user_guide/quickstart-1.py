...
# mk_path = Path.cwd() / 'user_guide/quickstart/quickstart_mk.tm'
script_dir = Path(__file__).resolve().parent

# 2. Check if we are running as a Sphinx build or a raw terminal debug session
if "plot_directive" in script_dir.parts:
    # Sphinx build environment context:
    # The true source file lives 3 levels up from the build static asset cache
    # build/plot_directive/user_guide/quickstart/ -> sphinx_files/user_guide/quickstart/
    project_root = script_dir.parents[4]
    mk_path = project_root / 'docs/online_documentation/sphinx_files/user_guide/quickstart/quickstart_mk.tm'
else:
    # Standalone terminal debugging or direct .rst source location context:
    mk_path = script_dir / 'quickstart_mk.tm'
